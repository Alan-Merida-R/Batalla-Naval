...
#define verdad 1
#define falso 0
void GuardarNuevoDato(char archivo[50],char dato[500],char finDatos[3]);
char *LeerArchivo (char archi[50],int Id,int datoRequerido);
void HacerArchivo(char nombreArchivoNuevo[50]);
void CambiarDatos(char archi[50],int Id,char dato[500],char finDatos[3]);
int ValidarDatosEnMismoRenglon(char archi[50],char Dato[50],char Dato2[50]);
int DamePosicionDelDato(char archi[50],char Dato[50],int rengloOColumna);
int teDaLaContrasena(char archi[50],char nombreDeUsuario[50],char contrase[50]);
...
void CambiarDatos(char archi[50],int Id,char dato[500],char finDatos[3]){
        ...
        if (CambioArchivo != NULL)
        {
            ...
            {
            		strcpy(cadena," ");
                    fgets(cadena,500,CambioArchivo);
                    idActual=idActual+1;
